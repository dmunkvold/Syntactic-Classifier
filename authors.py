...
def evaluate():
    ...
    builder = SampleBuilder(['Joyce','Haraway', 'Kant'])
    data, labels = builder.load_dataset_from_disk()
    eval_data = np.array(data[0:10000], dtype = np.float32)
    eval_labels = np.array(labels[0:10000], dtype = np.int32)
    train_data = np.array(data[10000:], dtype = np.float32)
    train_labels = np.array(labels[10000:], dtype = np.int32)
    print('eval', len(eval_data), len(eval_labels))
    print("Loaded training and eval data.")

    syntactic_classifier = learn.Estimator(
        model_fn=cnn_model_fn, model_dir="./model/syntactic-classifier_convnet_model103")
