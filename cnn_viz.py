...
def evaluate():
    ...
    eval_data = np.array(data[40000:], dtype = np.float32)
    eval_labels = np.array(labels[40000:], dtype = np.int32)
    train_data = np.array(data[:40000], dtype = np.float32)
    train_labels = np.array(labels[:40000], dtype = np.int32)
    print("Loaded training and eval data.")

    syntactic_classifier = learn.Estimator(
        model_fn=cnn_model_fn, model_dir="./model/syntactic-classifier_convnet_model79")
